class TransactionController < ApplicationController
  ...
  def email_offer
    ...
    @carts = Cart.joins(:item).where('buyer_id = ? AND user_id = ?', current_user.id, @seller.id)
    @prices = Price.all
  ...
  def send_email_offer
    ExampleMailer.offer_email(params[:buyer_id], params[:seller_id]).deliver_now

    # CREATE ITEMS IN OFFER

    # DESTROY ITEMS IN CART

    redirect_to "/buy", :notice => "All righty! Your email has been sent! Now just relax and wait for a reply!"
  end
end
